...
class TableManager {
  constructor(containerId, columns, defaultRow) {
    ...
    this.saveTimeout = null; // For debounced saving
  }
    ...
    this.table = new Handsontable(container, {
      ...
      afterChange: (changes, source) => {
        if (source !== "loadData") {
          ...
          this.debouncedSave(); // Use debounced save instead of immediate save
        }
      },
  ...
  updateValidationPanel(rowIndex, field, issue) {
    ...
    validationItem
      ...
      .addEventListener("click", (e) => {
        ...
        // Show the cargo input tab
        const cargoInputTab = document.getElementById("cargo-input-tab");
        if (cargoInputTab) {
          const tabInstance = new bootstrap.Tab(cargoInputTab);
          tabInstance.show();
        }
      });
  ...
  /**
   * Save data to localStorage with debouncing to avoid excessive saves
   */
  debouncedSave() {
    if (this.saveTimeout) {
  ...
  saveToLocalStorage() {
    ...
    try {
      // Filter out empty rows - only save items that have a name
      const dataToSave = this.data.filter((item) => item.name && item.name.trim() !== "");
      ...
      if (dataToSave.length > 0) {
        ...
        // If no data to save, remove the stored data
        localStorage.removeItem(CONFIG.STORAGE_KEYS.CARGO_DATA);
      }
    } catch (error) {
  ...
  loadFromLocalStorage() {
    try {
      ...
      if (savedData) {
        ...
        // Only return true if we have actual data with at least one named item
        if (Array.isArray(parsedData) && parsedData.length > 0) {
          // Check if there's at least one item with a name (meaningful data)
          const hasActualData = parsedData.some((item) => item.name && item.name.trim() !== "");
          ...
          if (hasActualData) {
            ...
            return true; // Only return true if we have meaningful data
          }
        }
      }
    } catch (error) {
  ...
  clearData() {
    ...
    if (this.table) {
      this.table.loadData(this.data);
      this.addHistoryItem(this.data.slice());
      this.saveToLocalStorage(); // Immediate save for clear action
    }
  }

  /**
   * Get the current data (only items with names)
   */
  getData() {
    return this.data.filter((item) => item.name && item.name.trim() !== "");
  }
